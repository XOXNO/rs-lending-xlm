...
//!
//! Every entrypoint renews the governance instance TTL, validates its
//! inputs, then forwards to the controller's thin owner-gated setters
//! through `ControllerAdminClient`. Invoker auth makes the controller
//! accept governance (its owner) as the direct cross-contract caller. The
//! controller keeps state-dependent invariants and event emission;
//! governance owns all input validation.
