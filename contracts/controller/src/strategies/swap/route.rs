//! Validation and invocation helpers for calling the swap aggregator router.
...
/// Validates that `amount_in` is positive and that `swap` is non-empty.
/// Panics with `GenericError::InvalidPayments` if `swap` is empty.
...
/// Calls `router.execute_strategy` on behalf of the current contract while
/// the flash-loan-ongoing flag is set, restoring its prior value afterward.
