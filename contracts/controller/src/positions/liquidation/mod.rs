//! Entry points for the liquidation flow: liquidating an undercollateralized
//! account against liquidator-supplied payments, and socializing bad debt either
//! through the standard dust-capped path or a forced insolvency path.

use crate::risk;
use common::validation::require_non_empty_payments;
mod apply;
mod bad_debt;
pub(crate) mod curve;
pub(crate) mod math;
mod plan;

pub(crate) use plan::execute_liquidation;

use common::errors::CollateralError;
use common::math::fp::Wad;
use common::types::{Account, AggregatedPayments, HubPayment};
use soroban_sdk::{assert_with_error, Address, Env, Vec};

use self::curve::is_socializable_bad_debt;
use crate::context::Cache;
use crate::events::LiquidationEvent;
use crate::payments;
use crate::positions::{persist_position_flow, PositionSides};
use crate::risk::validation;
use crate::storage;

/// Liquidates `account_id` by requiring `liquidator`'s authorization, collecting
/// `debt_payments` against a liquidation plan, and seizing collateral scaled to the
/// debt value actually received. Publishes a `LiquidationEvent`, persists the
/// updated account, and runs dust-capped bad-debt socialization if remaining debt
/// exceeds collateral and collateral ≤ `BAD_DEBT_USD_THRESHOLD`.
pub(crate) fn process_liquidation(
    env: &Env,
    liquidator: &Address,
    account_id: u64,
    debt_payments: &Vec<HubPayment>,
) {
    liquidator.require_auth();
    validation::require_not_flash_loaning(env);

    let mut account = storage::get_account(env, account_id);
    let aggregated = payments::aggregate_positive_payments(env, debt_payments);

    let mut cache = Cache::new(env);

    validate_liquidation_inputs(env, &account, liquidator, &aggregated);

    let liquidation_plan = plan::build_liquidation_plan(env, &account, &aggregated, &mut cache);

    let result = liquidation_plan.into_result();

    require_non_empty_payments(env, &result.repaid);

    let received_usd = apply::apply_liquidation_repayments(
        env,
        liquidator,
        &mut account,
        &result.repaid,
        &mut cache,
    );

    // Collateral is sized from the repayment the plan intended to collect. If a
    // debt token delivered less than was sent, shrink the seizure to match, or
    // the liquidator keeps collateral they did not pay for.
    let seized = math::scale_seizures_to_received(
        env,
        &result.seized,
        received_usd,
        math::sum_repaid_usd(env, &result.repaid),
    );
    apply::apply_liquidation_seizures(env, liquidator, &mut account, &seized, &mut cache);

    LiquidationEvent {
        liquidator: liquidator.clone(),
        account_id,
        repaid_usd_wad: result.max_debt_usd,
        bonus_bps: result.bonus_bps,
    }
    .publish(env);

    let post_totals = risk::calculate_account_risk_totals(
        env,
        &mut cache,
        &account.supply_positions,
        &account.borrow_positions,
    );

    persist_position_flow(
        env,
        account_id,
        &account,
        &mut cache,
        PositionSides::BOTH,
        false,
    );

    apply::check_bad_debt_after_liquidation(
        env,
        &mut cache,
        account_id,
        &account,
        post_totals.total_collateral,
        post_totals.total_debt,
    );
    cache.emit_position_batch(account_id, &account);
}

/// Asserts `aggregated` is non-empty and that `liquidator` is not the account's
/// owner. Panics with `CollateralError::SelfLiquidationNotAllowed` on
/// self-liquidation.
fn validate_liquidation_inputs(
    env: &Env,
    account: &Account,
    liquidator: &Address,
    aggregated: &AggregatedPayments,
) {
    require_non_empty_payments(env, aggregated);

    assert_with_error!(
        env,
        account.owner != *liquidator,
        CollateralError::SelfLiquidationNotAllowed
    );
}

/// Cleans up `account_id`'s bad debt after requiring `caller`'s authorization, using
/// the dust-capped socialization gate.
pub(crate) fn process_clean_bad_debt(env: &Env, caller: &Address, account_id: u64) {
    caller.require_auth();
    validation::require_not_flash_loaning(env);
    clean_bad_debt_standalone(env, account_id);
}

/// Gate determining which accounts qualify for bad-debt socialization.
enum BadDebtGate {
    /// Admits accounts whose remaining debt is socializable bad debt (see
    /// `is_socializable_bad_debt`).
    DustCapped,

    /// Admits any account whose total debt exceeds its total collateral.
    Insolvent,
}

impl BadDebtGate {
    /// Returns whether `total_debt` and `total_collateral` satisfy this gate's
    /// condition.
    fn admits(&self, total_debt: Wad, total_collateral: Wad) -> bool {
        match self {
            Self::DustCapped => is_socializable_bad_debt(total_debt, total_collateral),
            Self::Insolvent => total_debt > total_collateral,
        }
    }
}

/// Socializes `account_id`'s bad debt when `gate` admits its current debt and
/// collateral totals. Panics with `CollateralError::DebtPositionNotFound` if the
/// account has no borrow positions, or `CollateralError::CannotCleanBadDebt` if
/// `gate` does not admit the account.
fn socialize_bad_debt(env: &Env, account_id: u64, gate: BadDebtGate) {
    let mut cache = Cache::new(env);
    let account = storage::get_account(env, account_id);

    assert_with_error!(
        env,
        !account.borrow_positions.is_empty(),
        CollateralError::DebtPositionNotFound
    );

    let totals = risk::calculate_account_risk_totals(
        env,
        &mut cache,
        &account.supply_positions,
        &account.borrow_positions,
    );

    assert_with_error!(
        env,
        gate.admits(totals.total_debt, totals.total_collateral),
        CollateralError::CannotCleanBadDebt
    );

    bad_debt::execute_bad_debt_cleanup(
        env,
        &mut cache,
        account_id,
        &account,
        totals.total_debt.raw(),
        totals.total_collateral.raw(),
    );
}

/// Socializes `account_id`'s bad debt if it qualifies as dust-capped socializable
/// bad debt.
pub(crate) fn clean_bad_debt_standalone(env: &Env, account_id: u64) {
    socialize_bad_debt(env, account_id, BadDebtGate::DustCapped);
}

/// Forces socialization of `account_id`'s bad debt whenever its total debt exceeds
/// its total collateral, bypassing the dust-cap check.
pub(crate) fn process_force_socialize_bad_debt(env: &Env, account_id: u64) {
    validation::require_not_flash_loaning(env);
    socialize_bad_debt(env, account_id, BadDebtGate::Insolvent);
}
