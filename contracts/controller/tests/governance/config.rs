...
// Hub ids start at 1 (hub 0 is the implicit default) and each created hub is
// active on return.
#[test]
...
// Hub 0 is always active and never holds a registry entry.
#[test]
fn require_hub_active_passes_for_default_hub_zero() {
    let env = Env::default();
    let contract = new_controller(&env);
    env.as_contract(&contract, || {
