...
// ---------------------------------------------------------------------------
// Migration: the engine prices a legacy config through the lift
// ---------------------------------------------------------------------------

#[test]
fn test_the_engine_prices_an_unmigrated_legacy_config() {
    // What makes the upgrade safe to deploy before the migration finishes.
    let env = Env::default();
    at_now(&env);
    let (adapter, client) = register_redstone_feed(&env);
    publish(&client, &env, "USST", WAD, 0);
    ...
    in_contract(&env, || {
        ...
        let legacy = redstone_single(&env, &adapter, "USST", ASSET_CEILING);
        crate::storage::set_oracle_config(&env, &asset, &legacy);
