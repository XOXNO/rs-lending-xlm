...
fn reflector_rejects_future_timestamp() {
    ...
    assert!(OracleObservation::from_reflector(&env, 1_000, &data, 7).is_none());
}
...
fn reflector_accepts_fresh_positive_price() {
    ...
    let obs = OracleObservation::from_reflector(&env, 1_000, &data, 7)
...
fn reflector_rejects_non_positive_price() {
    let env = Env::default();
    assert!(
        OracleObservation::from_reflector(&env, 1_000, &reflector_data(&env, 0, 900), 7).is_none()
    );
    assert!(
        OracleObservation::from_reflector(&env, 1_000, &reflector_data(&env, -100, 900), 7)
            .is_none()
    );
}
...
fn multi_feed_accepts_fresh_positive_price() {
    ...
    let obs = OracleObservation::from_multi_feed(&env, 1_000, &data, 8)
        .expect("fresh positive price is acceptable");
    assert_eq!(obs.observed_at, 950);
    assert_eq!(obs.published_at, Some(900));
}
...
fn multi_feed_rejects_future_package_timestamp() {
    ...
    assert!(OracleObservation::from_multi_feed(&env, 1_000, &data, 8).is_none());
}
