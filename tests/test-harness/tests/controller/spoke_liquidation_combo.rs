...
// in spoke for the same price drop because the spoke threshold
...
fn test_spoke_threshold_supersedes_asset_threshold() {
    ...
    // Same setup under spoke — should be healthy because the spoke
...
// protocol's standard 15 % cap. Pins that the engine uses the spoke
...
fn test_spoke_bonus_bounded_by_category_bonus() {
    ...
    // but still inside spoke partial-liquidation band.
    ...
    // Spoke bonus (STABLECOIN_SPOKE) = 200 bps = 2 %. Engine clamps
...
fn test_spoke_liquidation_with_split_collateral() {
    ...
    // Drop USDC to push HF underwater. With spoke threshold 98 %:
...
// The existing `test_spoke_rejects_non_category_supply` test covers
// rejection from the start; this variant exercises the gate on a
// position that already has spoke positions open.
#[test]
