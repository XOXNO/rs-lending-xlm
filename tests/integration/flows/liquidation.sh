...
flow_caps() {
    ...
    if [ -z "${CAP_SETUP_DONE:-}" ]; then
        ...
        create_market CAP "$PRIMARY_HUB_ID" "$SAC_CAP" 7 "$(oracle_cfg_mock_single "$SAC_CAP")"  "$(asset_config_json 7000 7500 800)"
        create_market CAPC "$PRIMARY_HUB_ID" "$SAC_CAPC" 7 "$(oracle_cfg_mock_single "$SAC_CAPC")" "$(asset_config_json 7000 7500 800)"
        # Carol seeds CAP liquidity so the borrow-cap test has cash to draw.
        inv cap_seed "$CAROL" "$CONTROLLER" -- supply \
            --caller "$CAROL_ADDR" --account_id 0 --spoke_id "$PRIMARY_SPOKE_ID" \
            --assets "$(pay_vec "$PRIMARY_HUB_ID" "$SAC_CAP" $((20000 * LIQ_UNIT)))" >/dev/null || return 1
    ...
    inv cap_supply_tighten "$ADMIN" "$CONTROLLER" -- update_pool_caps \
        --hub_asset "$(hub_key "$PRIMARY_HUB_ID" "$SAC_CAP")" --supply_cap 1 --borrow_cap 0 >/dev/null
    xfail cap_supply_breach 'Error\(Contract, #105\)' "$BOB" "$CONTROLLER" -- supply \
        --caller "$BOB_ADDR" --account_id 0 --spoke_id "$PRIMARY_SPOKE_ID" \
        --assets "$(pay_vec "$PRIMARY_HUB_ID" "$SAC_CAP" $((1000 * LIQ_UNIT)))"
    inv cap_supply_reset "$ADMIN" "$CONTROLLER" -- update_pool_caps \
        --hub_asset "$(hub_key "$PRIMARY_HUB_ID" "$SAC_CAP")" --supply_cap 0 --borrow_cap 0 >/dev/null
    ...
    cap_acct=$(inv cap_coll_supply "$BOB" "$CONTROLLER" -- supply \
        --caller "$BOB_ADDR" --account_id 0 --spoke_id "$PRIMARY_SPOKE_ID" \
        --assets "$(pay_vec "$PRIMARY_HUB_ID" "$SAC_CAPC" $((3000 * LIQ_UNIT)))" | tr -d '"') || return 1
    inv cap_borrow_tighten "$ADMIN" "$CONTROLLER" -- update_pool_caps \
        --hub_asset "$(hub_key "$PRIMARY_HUB_ID" "$SAC_CAP")" --supply_cap 0 --borrow_cap 1 >/dev/null
    xfail cap_borrow_breach 'Error\(Contract, #106\)' "$BOB" "$CONTROLLER" -- borrow \
        --caller "$BOB_ADDR" --account_id "$cap_acct" \
        --borrows "$(pay_vec "$PRIMARY_HUB_ID" "$SAC_CAP" $((10 * LIQ_UNIT)))" --to null
