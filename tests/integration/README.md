...
| `governance.sh` | governance timelock e2e on the governance-owned controller: `deploy_controller` ownership (+#5 redeploy), resolver views, propose→cancel (Waiting→Unset), propose→await→`execute` (open executor) lifecycle (Waiting→Ready→Done), non-PROPOSER guard (#2000), owner pause + timelocked unpause forwarding |
