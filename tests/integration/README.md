...
| `liquidation.sh` | partial / full / bulk multi-debt liquidation, e-mode liquidation, clean_bad_debt socialization, healthy-account guards (#101); `flow_caps` — supply-cap (#105) + borrow-cap (#106) reverts on a dedicated capped market |
| `admin.sh` | pause gates (#1000/#1001), position limits (#36), param/config edits with read-back (#113 bounds), oracle tolerance (resolve→set, owner-auth guard), `disable_token_oracle` (#12/#2000), `set_min_borrow_collateral_usd` (set/read/#126 effect/reset/#116), keeper ops (+non-KEEPER #2000), revenue (+non-REVENUE #2000), e-mode admin lifecycle (#301), upgrade (pauses by design) + migrate + 2-step ownership round-trip |
