...
**Severity:** broken
**Rubric items failed:** [1]
**Why:** Lines 225-228 use bare `is_err()` rather than asserting `errors::INVALID_BORROW_PARAMS` (`CollateralError::InvalidBorrowParams = 116`, raised by `validation::validate_interest_rate_model` for the cap envelope). The test is the only direct guard for the documented Taylor-envelope cap; without an exact code, an unrelated panic in `upgrade_pool_params` would still satisfy `is_err()` and hide a regression.
...
**Why:** Line 248 only verifies that the call did not panic ("Did not panic — cap allows the boundary value."). There is no read of post-state to confirm the IRM was actually persisted. An accidental no-op (e.g. early return before the storage write) would still pass. Add a `pool_borrow_rate` snapshot or read the pool params back so the success path is anchored to observable storage.
...
**Severity:** weak
**Rubric items failed:** [3]
**Why:** Line 281 only calls `configure_market_oracle` and relies on the call not panicking. There is no assertion that `MarketConfig.cex_oracle`, `cex_symbol`, or `twap_records` reflect the input — the test would pass if the function silently dropped the input. Read back via `ctrl.get_market_config(&asset)` and assert at least one persisted field.
...
**Severity:** weak
**Rubric items failed:** [3]
**Why:** Line 298 only confirms `set_aggregator` does not panic. There is no readback: the test would pass if the new aggregator address was silently dropped. Storage exposes `ControllerKey::Aggregator` (controller/src/storage/instance.rs:47); a `t.env.as_contract` read or a follow-up oracle path that would reach the new aggregator is needed.
...
**Severity:** broken
**Rubric items failed:** [1]
**Why:** Lines 366-371 use bare `is_err()`. The `#[only_role]` macro (controller/src/router.rs:18) panics with `AccessControlError::Unauthorized = 2000` (vendor/openzeppelin/stellar-access/src/access_control/mod.rs:384). The inline comment claims "Soroban wraps cross-contract errors" but the role check is in-contract, so the SDK-generated `try_*` returns `Result<Result<_, Error>, InvokeError>` with the contract code preserved. Pin the code so an accidentally-removed `#[only_role]` would be caught.
...
+    assert!(
+        after_tolerance.oracle_config.fluctuation.first_upper_ratio_bps > 0,
...
(Reviewer should confirm the exact `OracleProviderConfig` field name; `fluctuation.first_upper_ratio_bps` is per `validation::validate_oracle_bounds` callers.)
...
**Severity:** broken
**Rubric items failed:** [1]
**Why:** Lines 229-233 use bare `is_err()`. The propagation path panics with `CollateralError::HealthFactorTooLow = 102` (controller/src/positions/supply.rs:508 inside `update_account_threshold`). Pin the code so an unrelated panic during threshold edit does not silently satisfy the assertion.
...
**Severity:** broken
**Rubric items failed:** [1]
**Why:** Lines 280-292 use two bare `is_err()` checks. Both `update_indexes` (router.rs:18) and `clean_bad_debt` (router.rs gating chain) are `#[only_role(caller, "KEEPER")]` and panic with `AccessControlError::Unauthorized = 2000`. Pin the exact code on both calls.
...
The two systemic weaknesses are concentrated in `admin_config_tests.rs` and `keeper_tests.rs`. The first is a recurring `assert!(result.is_err(), ...)` pattern across 10 tests (`test_edit_asset_config_rejects_threshold_lte_ltv`, `test_upgrade_pool_params_rejects_max_borrow_rate_above_cap`, `test_oracle_tolerance_validation`, all three `test_role_enforcement_*`, `test_clean_bad_debt_rejects_healthy`, `test_clean_bad_debt_rejects_above_threshold`, `test_update_account_threshold_rejects_low_hf`, `test_keeper_role_required`) — each masks a precise contract error code and would pass under any unrelated regression. The repeated comment about "Soroban wraps cross-contract errors" is misleading: the `#[only_role]` and `panic_with_error!` sites all live in the same controller WASM, so the SDK-generated `try_*` returns the expected `Result<Result<_, Error>, InvokeError>` shape and `assert_contract_error` works directly (e.g. `AccessControlError::Unauthorized = 2000`, `CannotCleanBadDebt = 114`, `HealthFactorTooLow = 102`, `InvalidLiqThreshold = 113`, `BadFirstTolerance = 207`, `InvalidBorrowParams = 116`). The second pattern is missing post-state on 7 success-path admin tests (`test_upgrade_pool_params_accepts_max_borrow_rate_at_cap`, `test_configure_market_oracle`, `test_set_aggregator`, `test_oracle_role_can_manage_oracle_endpoints`, `test_keepalive_pools_iterates_and_skips_unknown`, `test_keepalive_shared_state_bumps_emode_keys`, `test_upgrade_pool_admin_path`) — they confirm only that the admin call returns without panicking, leaving silent-no-op regressions undetected. The validation suite (`validation_admin_tests.rs`) is uniformly clean: every test uses `#[should_panic(expected = "Error(Contract, #N)")]` with the documented error code and the single success-path test (`test_validate_asset_config_accepts_flashloan_fee_at_cap`) reads the field back. No naming issues observed across the four files; severity 'nit' did not trigger.
