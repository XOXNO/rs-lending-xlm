...
**Totals:** confirmed=26 refuted=0 refined=2 new=2
...
**Severity:** weak
**Rubric items failed:** [4]
**Why:** Identical shape to `test_supply_isolated_account_single_asset` (which Phase 1 flagged): asserts `position_exists` and `supply_near` (lines 73-74) but never checks the wallet delta. A regression where the harness mints but the contract fails to pull the funds would still pass. Lines 64-75.

**Patch (suggested):**
```diff
--- a/test-harness/tests/emode_tests.rs
+++ b/test-harness/tests/emode_tests.rs
@@ -71,6 +71,7 @@ fn test_emode_supply_with_category_asset() {
...
**Severity:** weak
**Rubric items failed:** [4]
**Why:** Identical shape to `test_isolated_borrow_enabled_asset` (which Phase 1 flagged): asserts `position_exists`, `borrow_near`, `assert_healthy` (lines 95-97) but never checks Alice's wallet delta. The borrow path transfers borrowed USDT to Alice's wallet; a regression where the borrow accounting succeeds but the transfer to caller fails would still pass. Lines 82-98.

**Patch (suggested):**
```diff
--- a/test-harness/tests/emode_tests.rs
+++ b/test-harness/tests/emode_tests.rs
@@ -91,9 +91,12 @@ fn test_emode_borrow_with_category_asset() {
     t.create_emode_account(ALICE, 1);
     t.supply(ALICE, "USDC", 10_000.0);
...
+    assert!((usdt_wallet - 5_000.0).abs() < 1.0, "Alice should receive ~5000 USDT, got {}", usdt_wallet);
...
**Why:** Lines 159-167 use `std::panic::catch_unwind` over `t2.create_account_full(ALICE, 1, ..., true)`. `create_account_full` calls into the harness's own `create_account_direct` (test-harness/src/user.rs:53-107) which panics with a native Rust `assert!` (lines 68-71): `assert!(!(e_mode_category > 0 && is_isolated), ...)` — NOT through the contract. The actual `EModeError::EModeWithIsolated` (302) panic in `controller/src/positions/emode.rs:160` (and the `ensure_e_mode_compatible_with_asset` panic at line 49) is never executed. The test passes vacuously regardless of contract behavior. The fix drives the contract's supply path: an e-mode account that supplies an isolated asset must reject with code 302.
...
**Severity:** broken
**Rubric items failed:** [1]
**Why:** Lines 393-409 use `std::panic::catch_unwind` and bare `result.is_err()`. The contract path (`controller/src/config.rs:233`) panics with `CollateralError::InvalidLiqThreshold` (113); the `with_emode(...).build()` actually reaches that contract panic via `ctrl.add_e_mode_category(...)` (test-harness/src/context.rs:466), but the bare `is_err()` accepts any panic (e.g., a future fixture-build panic). The first `_t = ...build()` (line 390) is also dead setup. The test should call `try_add_e_mode_category` directly and pin the specific error code.
...
**Disposition:** refined
**Reviewer note:** The Phase 1 patch leaves `EModeCategoryPreset` unused in the import list (line 7 still imports it after the body no longer references it), which fails `-D warnings`. The rewritten patch drops `EModeCategoryPreset` from the `use` group as well. The contract-path call shape (`try_add_e_mode_category(&9000i128, &8000i128, &200i128)`) is preserved; the controller signature is `add_e_mode_category(env, ltv: i128, threshold: i128, bonus: i128) -> u32` (controller/src/config.rs:56) so the SDK-generated `try_*` accepts three `&i128`. The double-`Result` flatten matches the existing pattern in `test_account_owner_verified` and `test-harness/src/strategy.rs:249-260`.
...
**Severity:** broken
**Rubric items failed:** [1, 2]
**Why:** Lines 207-214 use `catch_unwind` over `create_account_full(ALICE, 1, ..., true)` — that hits the harness's native `assert!(!(e_mode_category > 0 && is_isolated), ...)` (test-harness/src/user.rs:68-71), NOT the contract's `EModeError::EModeWithIsolated` (302) panic. The test passes regardless of whether the contract enforces the rule. Drive the contract path: create an e-mode account, then try to supply an isolated asset — `ensure_e_mode_compatible_with_asset` (controller/src/positions/emode.rs:47-51) emits the proper code.
...
**Severity:** broken
**Rubric items failed:** [1]
**Why:** Lines 230-234 use bare `result.is_err()`. The contract path (`controller/src/strategy.rs:349-351`) emits `FlashLoanError::SwapCollateralNoIso` (404), which `try_swap_collateral` (test-harness/src/strategy.rs:249-260) surfaces as a contract error. `is_err()` would also accept oracle/tolerance/host failures and would not catch a regression that swapped the code or removed the isolation check entirely.
...
**Severity:** weak
**Rubric items failed:** [3]
**Why:** The test name promises "clears" the isolated tracker (i.e., back to zero), but only asserts `iso_debt_after < iso_debt_before` (line 305-310). The header comment (lines 301-303) explicitly says the bad-debt cleanup socializes the loss and the tracker must be cleared to zero. The bad-debt cleanup path (`controller/src/positions/liquidation.rs:507`) calls `clear_position_isolated_debt`, which routes through `adjust_isolated_debt_usd` (controller/src/utils.rs:159-186): when the repay USD value `>=` current isolated debt, the tracker is set to 0; sub-WAD residue is also zeroed (lines 181-183). For a $100 borrow that becomes bad debt, the tracker should be exactly 0. A regression that decrements by a tiny amount but never clears would still pass. Lines 274-311.
...
**Severity:** broken
**Rubric items failed:** [1]
**Why:** Lines 120-124 use bare `result.is_err()`. The harness's `try_remove_account` -> `remove_account_direct` (test-harness/src/user.rs:514-534) returns `CollateralError::PositionNotFound` (110) when supply or borrow positions exist. A specific contract-error assertion would catch a regression that returned the wrong code or no error.
...
--- a/test-harness/tests/decimal_diversity_tests.rs
+++ b/test-harness/tests/decimal_diversity_tests.rs
@@ -278,16 +278,24 @@ fn test_repay_cross_decimal() {
         .with_market(dai_18dec())
         .build();
...
+    assert!((dai_after_borrow - 5_000.0).abs() < 1.0, "Alice should have ~5000 DAI from borrow, got {}", dai_after_borrow);
     ...
     t.assert_healthy(ALICE);
+    // After partial repay the harness auto-mints 2_500 then the contract pulls
+    // 2_500, so Alice's wallet returns to ~5_000.
+    let dai_after_partial = t.token_balance(ALICE, "DAI18");
+    assert!((dai_after_partial - 5_000.0).abs() < 5.0, "DAI wallet should be ~5000 after partial repay, got {}", dai_after_partial);
...
+    // After full repay the harness auto-mints 3_000 (wallet ~8_000), the
+    // contract pulls ~2_500 (the residual debt) and refunds the ~500 surplus,
+    // leaving wallet at ~5_500. A regression that double-pulls would leave a
+    // much smaller balance; failing to refund would leave ~5_000.
+    let dai_wallet_final = t.token_balance(ALICE, "DAI18");
+    assert!(
+        dai_wallet_final > 5_400.0 && dai_wallet_final < 5_700.0,
+        "DAI wallet should be ~5500 after over-paid full repay (auto-mint 3000 minus actual ~2500 plus refund), got {}",
+        dai_wallet_final
+    );
 }
```
...
**Disposition:** refined
**Reviewer note:** Phase 1's final assertion `assert!(dai_wallet_final < 100.0, ...)` is incorrect. Tracing the auto-mint pattern: the borrow leg deposits ~5,000 DAI in Alice's wallet; partial repay auto-mints 2,500 → 7,500, contract pulls 2,500 → wallet returns to 5,000; full repay auto-mints 3,000 → 8,000, contract pulls ~2,500 (the residual debt) and refunds the ~500 overpayment via `pool::lib::repay` (pool/src/lib.rs:333-336) → wallet ~5,500. The Phase 1 patch would always fail. The refined diff replaces the bound with `> 5_400 && < 5_700`, and adds a partial-leg wallet check to catch double-pulls or skipped refunds.
...
The Phase 1 audit accurately identified three repeating issues across this domain.

First, four "rejection" tests for account-creation invariants (`test_emode_rejects_with_isolation`, `test_emode_deprecated_blocks_new_accounts`, `test_emode_remove_category_deprecates`, `test_isolated_rejects_emode`) catch panics from the test harness's own `create_account_direct` (test-harness/src/user.rs:53-107). That helper uses `env.as_contract` to write account state directly to storage and short-circuits validation with native Rust `assert!` calls (lines 68-71 and 78-81). The contract's `validate_e_mode_isolation_exclusion`, `ensure_e_mode_not_deprecated`, and `ensure_e_mode_compatible_with_asset` panics are never exercised; the tests pass vacuously regardless of whether the contract still enforces the rule. The fix is to drive the contract path via `try_supply` after creating an e-mode account against an isolated/deprecated configuration, then `assert_contract_error` with the specific code (302 / 301).

Second, multiple negative tests use bare `result.is_err()` or `catch_unwind` instead of `assert_contract_error(result, errors::SPECIFIC_CODE)` (`test_supply_position_limit_exceeded`, `test_isolated_rejects_swap_collateral`, `test_emode_rejects_threshold_lte_ltv`, `test_remove_rejects_with_positions`, `test_account_owner_verified`). A pin-coded check would catch regressions that swap an error code for a less informative one or accidentally route through a different validator. `invariant_tests.rs::test_position_limits_enforced` (line 217) demonstrates that this domain's specific codes are reachable via `assert_contract_error`.

Third, several success-path tests assert position/HF post-state but never verify wallet token-balance deltas (`test_supply_to_existing_account`, `test_supply_multiple_assets_bulk`, `test_supply_isolated_account_single_asset`, `test_supply_with_emode_category`, `test_isolated_borrow_enabled_asset`, `test_repay_cross_decimal`, `test_withdraw_cross_decimal_hf_check`, the cross-decimal liquidation pair, and `test_multiple_accounts_per_user`) — a regression where accounting succeeds but the underlying token transfer is dropped or mis-scaled would not be caught.

Phase 2 added two `new` entries (`test_emode_supply_with_category_asset`, `test_emode_borrow_with_category_asset`) that share the same shape as Phase 1 weak finds but were missed by the auditor, and refined two patches (`test_emode_rejects_threshold_lte_ltv` to remove the `EModeCategoryPreset` import that becomes unused; `test_repay_cross_decimal` to fix the wallet-final bound, which Phase 1 set incorrectly to `< 100.0` — the auto-mint pattern means the post-overpay wallet sits near 5,500 DAI, not near zero).
