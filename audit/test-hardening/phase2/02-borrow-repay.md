...
**Totals:** confirmed=43 refuted=0 refined=17 new=0
...
**Reviewer note:** Verified empirically by probing the same try_borrow shape — the contract returns `Err(Error(Contract, #109))` directly. `assert_contract_error(result, errors::POSITION_LIMIT_EXCEEDED)` works as advertised.
...
**Reviewer note:** Empirically confirmed the actual error is `Err(Error(Contract, #205))`. Replaced the auditor's `common::errors::OracleError::UnsafePriceNotAllowed as u32` (extra import, verbose) with the existing `errors::UNSAFE_PRICE` constant (=205) already exported from `test_harness::assert::errors`, plus `assert_contract_error` to match the project pattern. This avoids reaching into `common::errors` from a test file.
...
+    LIQUIDATOR,
+};
@@ -158,12 +159,9 @@ fn test_unsafe_price_blocks_borrow() {
...
**Reviewer note:** Replaced the auditor's `common::errors::OracleError::*` reference with the harness-exported `errors::UNSAFE_PRICE` for consistency with other tests in this file (single import block updated once).

**Patch (rewritten):**
```diff
--- a/test-harness/tests/oracle_tolerance_tests.rs
+++ b/test-harness/tests/oracle_tolerance_tests.rs
@@ -181,12 +181,9 @@ fn test_unsafe_price_blocks_borrow_debt_asset() {
...
**Reviewer note:** Same as above — use `errors::UNSAFE_PRICE` + `assert_contract_error` instead of reaching into `common::errors`.

**Patch (rewritten):**
```diff
--- a/test-harness/tests/oracle_tolerance_tests.rs
+++ b/test-harness/tests/oracle_tolerance_tests.rs
@@ -206,11 +206,8 @@ fn test_unsafe_price_blocks_withdraw_with_borrows() {
...
**Reviewer note:** Use `errors::UNSAFE_PRICE` + `assert_contract_error` (single shared import line covers all five `UNSAFE_PRICE` patches in this file).

**Patch (rewritten):**
```diff
--- a/test-harness/tests/oracle_tolerance_tests.rs
+++ b/test-harness/tests/oracle_tolerance_tests.rs
@@ -300,9 +300,9 @@ fn test_unsafe_price_blocks_liquidation() {
...
**Severity:** broken
**Rubric items failed:** [1]
**Why:** Generic `is_err()` (lines 326-329). The auditor proposed `errors::PRICE_FEED_STALE = 206`, but the empirically observed contract error is `OracleError::NoLastPrice = 210` — the test setup uses Soroban *temporary* storage in `MockReflector`, and after `advance_time_no_refresh(1000)` the temporary entry has expired (TTL), so `lastprice` returns `None`, which the controller maps to `NoLastPrice`. Pin the precise code.

**Disposition:** refined

**Reviewer note:** Probed the live test — `Err(Error(Contract, #210))` (NoLastPrice). The auditor's PRICE_FEED_STALE (206) patch would silently never trip in this configuration; the staleness check at oracle/mod.rs:182 is bypassed for risk-decreasing supply because `allow_unsafe_price=true`. The actual failure path is the missing `lastprice` from the expired temporary entry. Code 210 (NoLastPrice) is not currently in `errors::*`, so use `from_contract_error(210)` directly.

**Patch (rewritten):**
```diff
--- a/test-harness/tests/oracle_tolerance_tests.rs
+++ b/test-harness/tests/oracle_tolerance_tests.rs
@@ -322,11 +322,13 @@ fn test_stale_price_blocks_supply() {
...
+    assert_eq!(err, soroban_sdk::Error::from_contract_error(210),
+        "expected OracleError::NoLastPrice (210), got {:?}", err);
 }
```
...
**Severity:** broken
**Rubric items failed:** [1]
**Why:** Generic `is_err()` (line 343). The auditor proposed `PRICE_FEED_STALE = 206`; the empirical code is `NoLastPrice = 210` (same TTL-expiration mechanism as `test_stale_price_blocks_supply`).

**Disposition:** refined

**Reviewer note:** Probed live: `Err(Error(Contract, #210))`. Borrow's strict cache *would* trip 206 on a true staleness path, but in this harness the Mock's temporary storage expires before the staleness window matters, so `lastprice → None → NoLastPrice (210)`.

**Patch (rewritten):**
```diff
--- a/test-harness/tests/oracle_tolerance_tests.rs
+++ b/test-harness/tests/oracle_tolerance_tests.rs
@@ -340,8 +340,11 @@ fn test_stale_price_blocks_borrow() {
     // Borrow fails: stale price blocked for risk-increasing ops.
-    let result = t.try_borrow(ALICE, "ETH", 10.0);
-    assert!(result.is_err(), "borrow should fail with stale price");
+    let err = t
+        .try_borrow(ALICE, "ETH", 10.0)
+        .expect_err("borrow should fail under stale-mock conditions");
+    assert_eq!(err, soroban_sdk::Error::from_contract_error(210),
+        "expected OracleError::NoLastPrice (210), got {:?}", err);
 }
```
...
**Severity:** broken
**Rubric items failed:** [1]
**Why:** Generic `is_err()` (lines 357-360). Same correction as above: empirical code is `NoLastPrice = 210`, not `PRICE_FEED_STALE = 206`.

**Disposition:** refined

**Reviewer note:** Probed live: `Err(Error(Contract, #210))`.

**Patch (rewritten):**
```diff
--- a/test-harness/tests/oracle_tolerance_tests.rs
+++ b/test-harness/tests/oracle_tolerance_tests.rs
@@ -354,11 +354,11 @@ fn test_stale_price_blocks_withdraw_with_borrows() {
...
+        .expect_err("withdraw with borrows should fail under stale-mock conditions");
+    assert_eq!(err, soroban_sdk::Error::from_contract_error(210),
+        "expected OracleError::NoLastPrice (210), got {:?}", err);
 }
```
...
**Reviewer note:** Use `errors::UNSAFE_PRICE` + `assert_contract_error`.

**Patch (rewritten):**
```diff
--- a/test-harness/tests/oracle_tolerance_tests.rs
+++ b/test-harness/tests/oracle_tolerance_tests.rs
@@ -426,11 +426,8 @@ fn test_safe_price_below_aggregator_blocks_borrow() {
...
**Reviewer note:** The auditor's pattern `try_edit_oracle_tolerance(...).expect("invocation should reach the contract").expect_err(...)` is wrong: the raw client returns `Result<Result<(), Error>, Result<Error, InvokeError>>`, and a contract-level panic returns `Err(Ok(Error(Contract, #207)))` — so the outer `.expect()` panics on the very error we're trying to assert. Replace with the flatten-then-`assert_contract_error` shape used in `phase2/05-admin-keeper-config.md::test_oracle_tolerance_validation`.

**Patch (rewritten):**
```diff
--- a/test-harness/tests/oracle_tolerance_tests.rs
+++ b/test-harness/tests/oracle_tolerance_tests.rs
@@ -446,11 +446,12 @@ fn test_tolerance_config_rejects_first_below_min() {
...
**Reviewer note:** Same fix as `test_tolerance_config_rejects_first_below_min`.

**Patch (rewritten):**
```diff
--- a/test-harness/tests/oracle_tolerance_tests.rs
+++ b/test-harness/tests/oracle_tolerance_tests.rs
@@ -463,11 +463,12 @@ fn test_tolerance_config_rejects_first_above_max() {
...
**Severity:** broken
**Rubric items failed:** [1]
**Why:** Generic `is_err()` (lines 481-485). Expected `OracleError::BadLastTolerance = 208`. The call passes `&100, &100` — `last < MIN_LAST_TOLERANCE = 150` triggers `BadLastTolerance`.

**Disposition:** refined

**Reviewer note:** Empirically `Err(Ok(Error(Contract, #208)))`. The auditor's `.expect("invocation should reach the contract").expect_err(...)` chain panics on the contract error itself; replace with the flatten-then-assert pattern. Code 208 is not in `errors::*`, so use `soroban_sdk::Error::from_contract_error(208)` directly.

**Patch (rewritten):**
```diff
--- a/test-harness/tests/oracle_tolerance_tests.rs
+++ b/test-harness/tests/oracle_tolerance_tests.rs
@@ -479,11 +479,15 @@ fn test_tolerance_config_rejects_last_below_min() {
...
+        Err(e) => Err(e.expect("expected contract error, got InvokeError")),
+    };
+    let err = mapped.expect_err("last tolerance below 150 BPS should be rejected");
+    // OracleError::BadLastTolerance = 208.
+    assert_eq!(err, soroban_sdk::Error::from_contract_error(208),
+        "expected BadLastTolerance (208), got {:?}", err);
 }
```
...
**Severity:** broken
**Rubric items failed:** [1]
**Why:** Generic `is_err()` (lines 497-501). Expected `BadLastTolerance = 208`. (Note: the test's inline comment claims `MAX_LAST_TOLERANCE = 10000 BPS` but the constant is actually 5000 in `common/src/constants.rs:54`; the test still passes 11000 which is >MAX, so it correctly triggers 208.)

**Disposition:** refined

**Reviewer note:** Empirically `Err(Ok(Error(Contract, #208)))`. Auditor's `.expect()` chain wrong; flatten-then-assert and reference 208 via `from_contract_error`.

**Patch (rewritten):**
```diff
--- a/test-harness/tests/oracle_tolerance_tests.rs
+++ b/test-harness/tests/oracle_tolerance_tests.rs
@@ -495,11 +495,14 @@ fn test_tolerance_config_rejects_last_above_max() {
...
+        Err(e) => Err(e.expect("expected contract error, got InvokeError")),
+    };
+    let err = mapped.expect_err("last tolerance above MAX_LAST_TOLERANCE should be rejected");
+    assert_eq!(err, soroban_sdk::Error::from_contract_error(208),
+        "expected BadLastTolerance (208), got {:?}", err);
 }
```
...
**Reviewer note:** Empirically `Err(Ok(Error(Contract, #209)))`. Same flatten-then-assert pattern; code 209 not in `errors::*`, use `from_contract_error(209)`.

**Patch (rewritten):**
```diff
--- a/test-harness/tests/oracle_tolerance_tests.rs
+++ b/test-harness/tests/oracle_tolerance_tests.rs
@@ -511,11 +511,15 @@ fn test_tolerance_config_rejects_last_less_than_first() {
...
+        Err(e) => Err(e.expect("expected contract error, got InvokeError")),
+    };
+    let err = mapped.expect_err("last tolerance < first tolerance should be rejected");
+    // OracleError::BadAnchorTolerances = 209.
+    assert_eq!(err, soroban_sdk::Error::from_contract_error(209),
+        "expected BadAnchorTolerances (209), got {:?}", err);
 }
```
...
**Reviewer note:** Auditor's storage assertion is wrong on two counts: (a) tolerance is stored under `ControllerKey::Market(asset)` as part of `MarketConfig.oracle_config.tolerance`, not under a (nonexistent) `ControllerKey::OracleTolerance(asset)` key; (b) `OraclePriceFluctuation` has fields `first_upper_ratio_bps` / `first_lower_ratio_bps` / `last_upper_ratio_bps` / `last_lower_ratio_bps` (precomputed ratio bounds), not `first_tolerance_bps` / `last_tolerance_bps`. `validate_and_calculate_tolerances` (controller/src/config.rs:422-444) transforms the raw BPS inputs into ratio bounds on write, so verifying persistence requires checking the recomputed bounds. With first=300 BPS the upper ratio is `10000 + 300 = 10_300`; with last=600 BPS the upper ratio is `10000 + 600 = 10_600`.

**Patch (rewritten):**
```diff
--- a/test-harness/tests/oracle_tolerance_tests.rs
+++ b/test-harness/tests/oracle_tolerance_tests.rs
@@ -526,9 +526,21 @@ fn test_tolerance_config_valid_update() {
...
+
+    // Verify the new tolerance is in storage. `edit_oracle_tolerance` stores
+    // pre-computed ratio bounds (not raw BPS) on the `MarketConfig`. With
+    // first=300 BPS the upper ratio is 10_300; with last=600 BPS the upper
+    // ratio is 10_600 (see `validate_and_calculate_tolerances` /
+    // `calculate_tolerance_range`).
+    let market: common::types::MarketConfig = t.env.as_contract(&t.controller, || {
+        t.env.storage().persistent()
+            .get(&common::types::ControllerKey::Market(asset.clone()))
+            .expect("market config must be stored")
+    });
+    assert_eq!(market.oracle_config.tolerance.first_upper_ratio_bps, 10_300);
+    assert_eq!(market.oracle_config.tolerance.last_upper_ratio_bps, 10_600);
 }
```
...
**Severity:** broken
**Rubric items failed:** [1]
**Why:** Generic `is_err()` (lines 593-597). After disabling the oracle the borrow path must fail with a specific code; the test must pin it down so a regression that fails for a different reason (e.g. INSUFFICIENT_COLLATERAL) cannot pass.

**Disposition:** refined

**Reviewer note:** The auditor expected `OracleError::OracleNotConfigured = 216`, but the empirical contract error is `GenericError::PairNotActive = 12`. `disable_token_oracle` (controller/src/config.rs:596-600) only flips `MarketStatus` to `Disabled`; on the next price read, `token_price` (oracle/mod.rs:33-39) panics with `GenericError::PairNotActive` for any cache where `allow_disabled_market_price = false`, which is the default for borrow's `ControllerCache::new(env, false)`. Code 12 is not currently in `errors::*`; reference it via `from_contract_error(12)`.
...
-    );
+    // disable_token_oracle flips MarketStatus to Disabled. Borrow uses the
+    // strict cache (allow_disabled_market_price=false), so token_price
+    // panics with GenericError::PairNotActive (12).
+    let err = t
+        .try_borrow(ALICE, "ETH", 1.0)
+        .expect_err("borrow should fail when collateral oracle is disabled");
+    assert_eq!(err, soroban_sdk::Error::from_contract_error(12),
+        "expected GenericError::PairNotActive (12), got {:?}", err);
 }
```
...
--- a/test-harness/tests/oracle_tolerance_tests.rs
+++ b/test-harness/tests/oracle_tolerance_tests.rs
@@ -696,11 +696,8 @@ fn test_mixed_tolerance_states() {
...
--- a/test-harness/tests/oracle_tolerance_tests.rs
+++ b/test-harness/tests/oracle_tolerance_tests.rs
@@ -747,11 +747,8 @@ fn test_liquidation_dos_flash_crash() {
...
The auditor correctly identified the dominant defect — **error-code laxity** across the oracle-tolerance tests — and the secondary pattern of **success-path under-assertion**. However, the auditor's empirically-untested patches landed on the wrong codes in three places:

1. **Stale-price tests** (`test_stale_price_blocks_supply` / `_borrow` / `_withdraw_with_borrows`) trip `OracleError::NoLastPrice = 210`, not `PriceFeedStale = 206`. The mock reflector uses Soroban *temporary* storage; `advance_time_no_refresh` expires the entry before the staleness check fires, so `lastprice` returns `None` and the controller panics with NoLastPrice. The auditor's 206 patches would fail in Phase 3.
2. **`test_disable_token_oracle_blocks_operations`** trips `GenericError::PairNotActive = 12`, not `OracleError::OracleNotConfigured = 216`. `disable_token_oracle` flips `MarketStatus`, and `token_price` panics with PairNotActive for the strict-cache borrow path.
3. **All five `test_tolerance_config_*` patches** use `.expect("invocation should reach the contract").expect_err(...)` — wrong: the raw `ctrl.try_edit_oracle_tolerance` returns `Result<Result<(), Error>, Result<Error, InvokeError>>`, and a contract panic surfaces as `Err(Ok(Error(...)))`, so the outer `.expect` panics on the very error we want to assert. Replaced with the flatten-then-`assert_contract_error` shape already adopted in `phase2/05-admin-keeper-config.md::test_oracle_tolerance_validation`.

Additional refinements: cleaned up six `UNSAFE_PRICE` patches to use the existing `errors::UNSAFE_PRICE` constant (already exported from `test_harness::assert::errors`) and the `assert_contract_error` helper, instead of importing `common::errors::OracleError` from test files. The `test_tolerance_config_valid_update` storage assertion was rewritten to read `MarketConfig.oracle_config.tolerance.{first_upper_ratio_bps, last_upper_ratio_bps}` (the actual schema) rather than nonexistent `first_tolerance_bps` / `last_tolerance_bps` fields under a nonexistent `ControllerKey::OracleTolerance(asset)` key. The auditor's empirical claim about `assert_contract_error(result, errors::POSITION_LIMIT_EXCEEDED)` working for the borrow path is correct: probed the live test, the contract returns `Err(Error(Contract, #109))` directly, just as in `invariant_tests.rs:217` and `strategy_edge_tests.rs:1195`. All other Phase 1 dispositions (severities and rationales) hold.
