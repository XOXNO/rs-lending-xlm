...
fn isqrt_is_the_integer_floor_of_the_root(e: Env, a: u64, b: u64) {
    ...
    // Known model gap, so this rule runs from its own conf (see
    // `certora/README.md`). The prover models `context/obj_cmp` on two U256
    // objects as "equal digests give 0, otherwise a havoc in {1, -1}", so every
    // ordering comparison in `isqrt_of_product` (`n <= one`, `while y < x`) is
    // nondeterministic. Until U256 ordering is modelled, any verdict here is
    // about the model, not the code.
