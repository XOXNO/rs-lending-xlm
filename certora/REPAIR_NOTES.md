...
The certora **compile gate** is `certora/compile_all.sh`:
...
- Job `compile-check` runs `./certora/compile_all.sh` **without
...
`certora/controller/{spec,harness}/**`. ~1 file-wide find/replace.
...
All in `certora/controller/`. Same edit everywhere.
...
| controller group A (51 E0433) | global `controller::`→`crate::` sweep across `certora/controller/{spec,harness}` + `common::types::PriceFeedRaw`→`crate::types` | ~30 min (one find/replace + spot-check) |
