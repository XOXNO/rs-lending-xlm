...
selected accounting bounds. Within one rule, each asset has one price and each
market one index pair (harness/ghost_prices.rs), so health comparisons use one
valuation snapshot. Cross-contract claims remain conditional on the trusted
pool and price summaries described below.

Assumptions
-----------
- Pool calls resolve to the summaries in shared/summaries/pool.rs, through
  harness/external/pool.rs. Those summaries are asserted assumptions, not
  proved refinements, so a controller verdict that reaches one is conditional
  evidence. Position NFT ownership is a ghost map
...
boundary_rules.rs
  boundary-oracle.conf, boundary-bad-debt-sanity.conf
...
liquidation_rules.rs
  liquidation.conf, liquidation-additivity.conf, liquidation-bonus.conf,
...
solvency_rules.rs
  solvency-borrow.conf, solvency-index.conf, solvency-borrow-reverts.conf,
  solvency-reserves-reverts.conf, repay-gates-reverts.conf,
  solvency-borrow-reverts-sanity.conf, solvency-reserves-reverts-sanity.conf,
  solvency-sanity.conf, index-cache-sanity.conf
...
strategy_rules.rs
  ...
  strategy-repay-collateral.conf (heavy profile: full withdraw, swap and repay
  path), strategy-swap-collateral.conf, strategy-swap-debt.conf,
...
Oracle
  ...
  consume price_feed_summary (shared/summaries/mod.rs): a positive price per
  asset that never fails, replayed within a rule by harness/ghost_prices.rs.

Support modules (no rules)
  spec/compat.rs — single-asset ABI shims for multi-asset entrypoints
  spec/health_ghost.rs — ghost record of the post-pool solvency gate
  spec/fixture.rs — protocol, market and account seeding
  spec/mod.rs — module mount
  harness/ — storage helpers, the per-rule price and index snapshot, and the
    pool, price-aggregator and position-nft summaries
...
completes the same fixture: a `<rule>_fixture_completes` twin in the sibling
`-reverts-sanity.conf`, or an existing witness listed in `EXISTING_WITNESS` in
certora/scripts/check_orphans.py, which enforces the pairing. See "Sanity
checking on WASM" in certora/README.md.
...
Runtime cross-references: contracts/controller/src, contracts/pool/src and
tests/test-harness.
