...
impl Metrics {
    pub fn new() -> Result<Self> {
        ...
        Ok(Self {
            ...
            oracle_anchor_price_usd: register_gauge_vec(&registry, "lending_oracle_anchor_price_usd", "Anchor source USD price", ORACLE_LABELS)?,
            ...
            oracle_healthy: register_gauge_vec(&registry, "lending_oracle_healthy", "1 if the aggregated price view resolved, else 0", ORACLE_LABELS)?,
            oracle_max_stale_seconds: register_gauge_vec(&registry, "lending_oracle_max_stale_seconds", "Configured max price staleness (s)", ORACLE_LABELS)?,
            ...
            oracle_price_timestamp: register_gauge_vec(&registry, "lending_oracle_price_timestamp_seconds", "Feed price timestamp (Unix s)", ORACLE_LABELS)?,
            oracle_seconds_until_stale: register_gauge_vec(&registry, "lending_oracle_seconds_until_stale", "Seconds until the feed goes stale (negative if already stale)", ORACLE_LABELS)?,
