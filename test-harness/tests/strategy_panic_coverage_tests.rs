...
//! exercised end-to-end. Each test here pins the exact contract error
//! code so that a regression which substitutes a different error (or
//! worse, silently succeeds) breaks CI.
//!
...
fn test_multiply_third_token_payment_without_convert_steps_rejects() {
    ...
    // Mint some WBTC for Alice (the third token).
...
// No prior test exercised this path.
// ---------------------------------------------------------------------------
#[test]
fn test_multiply_existing_account_mode_mismatch_rejects() {
    ...
    // Now try to reuse the Multiply account with Long mode → must reject.
    let ctrl = t.ctrl_client();
    let result = ctrl.try_multiply(
        &alice,
        &account_id,
...
//   1. handle_create_borrow_strategy(new_debt_token) — flash-borrows new token
//   2. swap_tokens(new_debt -> existing_debt)
//   3. borrow_positions.get(existing_debt_token) ← panics if missing (line 284)
// So we must fund the router correctly AND ensure the flash borrow succeeds.
// ---------------------------------------------------------------------------
#[test]
fn test_swap_debt_existing_position_missing_rejects() {
    ...
    // Alice supplies USDC + borrows WBTC. She does NOT borrow ETH.
    t.supply(ALICE, "USDC", 100_000.0);
    t.borrow(ALICE, "WBTC", 0.01);

    // Swap direction: new=WBTC, swap WBTC -> ETH. Router must be funded with ETH.
    t.fund_router("ETH", 0.5);
    let steps = build_swap_steps(&t, "WBTC", "ETH", 5_000_000);
    // existing=ETH (not held by Alice), new=WBTC (Alice already holds WBTC debt,
    // but swap_debt doesn't require the NEW debt be missing — only the existing).
...
fn test_repay_debt_with_collateral_missing_collateral_rejects() {
    ...
    // Alice has USDC collateral + ETH debt, but no WBTC collateral.
    t.supply(ALICE, "USDC", 100_000.0);
...
// ---------------------------------------------------------------------------
// strategy.rs — DebtPositionNotFound in repay_debt_with_collateral (regression)
//
// The audit surfaced two bugs here:
//   1. swap_tokens received `collateral_amount` (the REQUESTED amount), not the
//      actual withdrawal delta. Same M-11 pattern that had been fixed for
//      swap_collateral but not ported here.
...
//
// Both fixed in strategy.rs: both positions are validated up-front, then the
// function measures the actual withdrawal delta and feeds it into swap_tokens.
// This test pins the DebtPositionNotFound guard.
// ---------------------------------------------------------------------------
#[test]
fn test_repay_debt_with_collateral_missing_debt_rejects() {
    ...
    // Alice supplies USDC but borrows ETH (not WBTC).
    t.supply(ALICE, "USDC", 100_000.0);
    t.borrow(ALICE, "ETH", 1.0);
...
//
// close_position=true must be rejected if the account still has debt. This
// is explicitly guarded at line 599-601; no prior test exercised it.
// ---------------------------------------------------------------------------
#[test]
fn test_repay_debt_with_collateral_close_with_remaining_debt_rejects() {
    ...
    // Alice has a large ETH debt — a small repay won't zero it out.
    t.supply(ALICE, "USDC", 100_000.0);
    t.borrow(ALICE, "ETH", 5.0);

    // Repay only a tiny fraction — Alice will still owe ETH.
...
// strategy.rs:80-82 — payment is added directly to collateral_amount.
...
fn test_multiply_with_collateral_token_initial_payment() {
    ...
    // Mint 500 USDC to Alice so she can pay in with the SAME token she'll use as collateral.
    usdc_market
        .token_admin
    ...
    // Total collateral must equal initial payment (500) + swapped amount (~3000).
    ...
    // Borrow is only the flash-loaned debt; initial collateral payment does NOT
    // show up as additional borrow (distinguishes Case 1 from Case 2).
...
// ---------------------------------------------------------------------------
// Case 3: initial_payment is a third token, convert_steps supplied.
// strategy.rs:87-100 — payment is swapped to collateral via convert_steps.
// No prior directed happy-path test.
// ---------------------------------------------------------------------------
#[test]
fn test_multiply_with_third_token_initial_payment_swaps_via_convert_steps() {
    ...
    // Main debt swap (ETH -> USDC) and initial-payment convert (WBTC -> USDC).
    // The mock aggregator funds each side independently, so fund for BOTH.
...
//
// Soroban rolls back state on contract panics, so the allowance approved
// by swap_tokens line 451 is undone atomically. We verify it post-op.
// Distinct from fuzz_strategy_flashloan prop 2 which covers the
// success-path zeroing (line 483).
// ---------------------------------------------------------------------------
#[test]
fn test_swap_tokens_allowance_remains_zero_after_overpull_rejection() {
    ...
    // Pre-seed output so the bad router can transfer (before it tries to over-pull).
    ...
    // After rollback the controller's ETH allowance on the bad router must be
    // zero. A regression that leaked the pre-approved allowance would expose
    // the controller to future drains.
    let eth = t.resolve_asset("ETH");
...
//
// swap_debt + swap_collateral already cover this path in edge_tests; the
// multiply reuse path (line 135-137) was not directly tested.
// ---------------------------------------------------------------------------
#[test]
