...
fn test_multi_user_lending_cycle() {
    ...
    // -- Week 3: Price drop makes Carol liquidatable --
    ...
    // Healthy users should remain healthy
    t.assert_healthy(ALICE);
    t.assert_healthy(BOB);
    t.assert_healthy(DAVE);
    ...
    // All accounts with borrows should be healthy
    t.assert_healthy(ALICE);
    t.assert_healthy(BOB);
    t.assert_healthy(DAVE);
    t.assert_healthy(EVE);
...
fn test_full_exit_solvency() {
    ...
    // Use very large overpayment to ensure full repay
    ...
    // Pool should remain solvent (reserves >= 0)
    ...
    // Revenue should be positive (interest was generated)
    assert!(
...
fn test_cascading_liquidations_stability() {
    ...
    // Create users with different HF levels.
    // All supply ETH and borrow USDC.
    // ETH price = $2000, LTV = 75%, liq threshold = 80%
    //
    ...
    // Eve should remain untouched (no wrongful liquidation)
...
fn test_interest_accrual_consistency() {
    ...
    // Alice supplies ETH as collateral and borrows ETH from the same pool.
    // Bob supplies ETH to the pool so Alice can borrow from it.
    // This way Bob's ETH supply earns interest from Alice's ETH borrow.
    t.supply(ALICE, "USDC", 100_000.0);
    ...
    for (i, &interval) in intervals.iter().enumerate() {
        ...
        // Supply balance should increase (earning interest from borrowers)
        assert!(
    ...
    // Verify protocol revenue was generated
    ...
    // reserve_factor = 1000 BPS = 10%
    // This is approximate due to compound interest effects
    let total_supply_interest = t.supply_balance(BOB, "ETH") - 100.0;
    let _total_borrow_interest = t.borrow_balance(ALICE, "ETH") - 25.0;

    // Supply interest should be positive (suppliers earn)
    assert!(
...
fn test_keeper_index_freshness_matters() {
    ...
    // Both debts should be > 10 ETH (interest accrued)
    assert!(
        debt_a > 10.0,
    ...
    // Both revenues should be positive
    assert!(
        revenue_a > 0,
    ...
    // the protocol tracks interest via index updates. Without syncs, revenue
    // only materializes when indexes are updated. Daily syncs capture the
    // full compound interest curve; a single sync underestimates.
    //
    // Scenario B (daily syncs) should accrue >= Scenario A (single sync)
    // for both debt and revenue.
    assert!(
        debt_b >= debt_a,
