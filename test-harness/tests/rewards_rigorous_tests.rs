...
// ===========================================================================
// Rigorous add_rewards tests — verify the supply index math is correct.
//
// Formula: new_index = old_index * (1 + rewards / total_supplied_value)
// where total_supplied_value = supplied_scaled * old_index / RAY
//
// Key properties:
// 1. Supply index increases by exactly rewards / total_supplied_value
// 2. Each supplier's balance increases proportionally to their share
...
fn test_add_rewards_distributed_proportionally() {
    ...
    // Alice should get 60% of 10,000 = 6,000
    assert!(
    ...
    // Bob should get 40% of 10,000 = 4,000
    assert!(
    ...
    // Total rewards distributed should equal the amount added
...
fn test_add_rewards_does_not_affect_borrow_index() {
    ...
    // Sync indexes first so they're clean
    t.advance_and_sync(days(1));

    let (_, bi_before) = get_indexes(&t, "USDC");

    // Add large rewards to USDC pool
    t.add_rewards("USDC", 50_000.0);

    let (_, bi_after) = get_indexes(&t, "USDC");

    // `add_rewards` should not materially change the borrow index. The call
    // runs `global_sync`, but the extra drift here is negligible.
...
fn test_add_rewards_compounds_over_multiple_calls() {
    ...
    // Each addition should increase balance
    assert!(
    ...
    // Total should be approximately 100,000 * 1.01^3 ≈ 103,030.1
...
// ---------------------------------------------------------------------------
// 5. Rewards with zero supply is a no-op (index unchanged)
// ---------------------------------------------------------------------------
...
fn test_add_rewards_noop_when_no_supply() {
    ...
    // With zero supply, `add_rewards` transfers tokens but should leave the
    // supply index unchanged.
    t.add_rewards("USDC", 1_000.0);

    let (si_after, _) = get_indexes(&t, "USDC");

    // Index should be unchanged (rewards distributed to 0 suppliers = no-op)
    assert_eq!(
        si_before, si_after,
...
fn test_rewards_plus_interest_compound() {
    ...
    // Interest should be positive (from borrows)
    assert!(
        interest_earned > 0.0,
    ...
    // Rewards should add ~5,000 to Alice (she's the sole USDC supplier)
    assert!(
...
fn test_large_rewards_accounting_stable() {
    ...
    // Balance should be ~101,000 (1,000 supply + 100,000 rewards)
    assert!(
...
fn test_rewards_after_interest_proportional() {
    ...
    // Now balances are no longer exactly 75k/25k due to interest
    let a_before_reward = t.supply_balance(ALICE, "USDC");
    let b_before_reward = t.supply_balance(BOB, "USDC");

    // Add rewards — should still split proportionally based on current share
    t.add_rewards("USDC", 10_000.0);

    let a_reward = t.supply_balance(ALICE, "USDC") - a_before_reward;
    let b_reward = t.supply_balance(BOB, "USDC") - b_before_reward;

    // Ratio should still be 3:1 (proportional to scaled amounts, which haven't changed)
