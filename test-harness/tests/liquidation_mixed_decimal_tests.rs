...
fn test_liquidation_two_collaterals_6dec_18dec_debt_8dec() {
    ...
    // Alice supplies 50/50 split across 6-dec and 18-dec
    ...
    // Price drop: WBTC rises to $70,000 making debt = $8,750 > $8,000 threshold
    ...
    // Both seizures should be roughly proportional (within 20% of each other)
...
//    Tests that small proportional seizure on 18-dec is non-zero.
// ---------------------------------------------------------------------------
...
fn test_liquidation_asymmetric_90pct_6dec_10pct_18dec() {
    ...
    // SOL price rises to $175 → debt = $8,750 > threshold $8,000
    ...
    // USDC seizure should be ~9x DAI seizure (proportional to 90/10 split)
...
//    Tests correct conversion when debt payments span decimal ranges.
// ---------------------------------------------------------------------------
...
fn test_liquidation_multi_debt_different_decimals() {
    ...
    // Crash DAI → $0.85 → collateral=$17,000, threshold=$13,600, debt=$14,000 → underwater
...
fn test_bad_debt_cleanup_mixed_decimals() {
    ...
    // Liquidate to trigger bad debt path (collateral < $5)
    t.liquidate(LIQUIDATOR, ALICE, "DAI18", 10.0);

    // After liquidation + bad debt cleanup, the account is removed entirely.
    // The liquidator should have received some DAI back (refund from overpayment
    // or the repaid amount was capped). Just verify no panic occurred.
...
//    Verifies fee doesn't underflow for 6-dec or overflow for 18-dec.
// ---------------------------------------------------------------------------
...
fn test_liquidation_protocol_fee_cross_decimal() {
    ...
    // Collateral should decrease (seizure happened)
    assert!(
...
//    If this exceeds Soroban budget, it reveals the max position complexity.
// ---------------------------------------------------------------------------
