//! Phase-B differential fuzz harness — production liquidation math vs
//! `num_rational::BigRational` reference.
//!
//! For each randomly-generated scenario:
//!   1. Spin up a USDC + ETH `LendingTest`.
//!   2. Set Alice up with an underwater position (drop ETH price after she
...
//!      fees to the reference result with a documented ulp bound.
//!
//! The primary comparison is **total debt reduction in USD WAD**. This is the
//! most sensitive aggregate across the full liquidation chain (HF → bonus →
//! ideal repayment → seizure → rescale). Per-asset seizure and fees are
//! additionally compared for the single-collateral scenario exercised here.
//!
//! Run:
...
//! token* bound is the tightest — production and reference agree to within a
//! handful of ulps at the token level. The USD-WAD aggregates are looser
//! because the production path re-derives USD totals through
//! `total_collateral_in_usd` / `total_borrow_in_usd`, each of which
//! independently rounds `scaled * index * price` per position, while the
//! reference aggregates the exact pre-liquidation USD value and multiplies by
//! `one_plus_bonus` with no rounding. For a $40 seizure we've observed drifts
//! up to ~1e15 WAD (= 0.001 USD), which is ~2.5e-5 relative. We therefore use
//! a relative bound of 1e-3 (0.1%) on USD aggregates — a generous envelope
//! that still catches real bugs (anything > 0.1% drift would indicate a
//! systematic rounding-direction error, not the 0.5-ulp-per-op accumulation
//! this harness is calibrated to tolerate).
//!
//! Per-asset token comparison (the more physically meaningful one — this is
//! what the protocol actually transfers) keeps a tight 1e-6 relative bound.
...
/// Allowed drift in USD-WAD space (10^-17 USD). Much smaller than dust.
const ULP_BOUND_USD_WAD: i128 = 10;

/// Allowed drift in token-unit space for 7-decimal assets. 10 micro-units
/// (10^-6 of a token). Protocol never round-trips at finer precision than this.
const ULP_BOUND_TOKENS: i128 = 50;

fn target_hf_wad() -> BigRational {
    // 1.02 in WAD scale. We pass this explicitly for documentation; the
    // reference internally uses the same primary target.
...
proptest! {
    ...
    fn prop_liquidation_matches_bigrational_reference(
        ...
        if t.health_factor_raw(ALICE) >= WAD {
            // Not underwater — scenario doesn't apply.
            return Ok(());
        }

        // Skip bad-debt territory: the reference implementation does not
        // model bad-debt socialization (full write-off when debt > collateral
        // AND collateral <= $5 WAD, or when seizure would drain all
        // collateral). Production *does* model it, so any scenario where
        // this triggers would produce a legitimate divergence that's
        // outside the differential scope (see plan "Scope boundary").
        //
        // Heuristic: if total_debt > total_collateral *or* the seizure
        // amount (ideal_repayment * (1+max_bonus)) would drain >90% of
        // collateral, skip. 90% gives headroom for the bad-debt path to
        // kick in after actual seizure.
        let coll_wad = t.total_collateral_raw(ALICE);
        let debt_wad = t.total_debt_raw(ALICE);
        if debt_wad >= coll_wad {
            // Already underwater past the collateral value — bad-debt
            // socialization very likely fires after any sizeable liquidation.
            return Ok(());
        }
        // Also skip the near-bad-debt zone: collateral < 115% of debt
        // (bonus-adjusted safety margin). This conservatively avoids cases
        // where the 15%-max-bonus seizure leaves collateral below the
        // bad-debt threshold mid-liquidation.
        ...
        // Snapshot the actual USDC supply balance *as the protocol sees it*
        // right before liquidation — `supply_usdc * 10^7` is the deposit
        // intent, but after the initial supply the scaled/index round-trip
        // can differ by a ulp. This is the true baseline for per-asset
        // seizure comparison.
        let usdc_supply_before_tokens = t.supply_balance_raw(ALICE, "USDC");
        let liq_res = t.try_liquidate(LIQUIDATOR, ALICE, "ETH", repay_amt);
        if liq_res.is_err() {
            // Protocol can reject for e.g. HF>=1 after rounding; if the
            // reference also derived zero-repayment we're consistent.
            if ref_total_repaid_usd_wad == 0 {
                return Ok(());
            }
            // Otherwise we've drifted — but only flag if the reference
        ...
        // `total_collateral_in_usd` / `total_borrow_in_usd`, each of which
        // independently rounds `scaled * index * price` per position; the
        // reference uses exact rationals throughout. The pre-liquidation USD
        // is therefore already drifted by multiple ulps vs the reference, and
        ...
        // (sampled right before `try_liquidate`). After, the balance is
        // reduced by the full `capped_amount` (base + bonus): the base goes
        // to the liquidator, the fee portion to pool revenue.
        ...
        // positions where rounding dominates the signal. We allow 0.5%
        // as a comfortable envelope; anything larger is a real finding.
        let usdc_rel_ok =
