...
/// Minimum allowed supply_index to keep scaled-amount math well-conditioned.
/// 10^18 in RAY (= 10^-9 decimal). Below this, `calculate_scaled_supply`
/// division produces astronomical scaled values that can overflow i128.
...
/// Used for fees that arrive as token amounts (protocol_fee from liquidation, flash loan fee).
...
pub fn add_protocol_revenue_ray(cache: &mut Cache, fee: Ray) {
    ...
    // Skip revenue accrual if supply_index is at or near the safety floor —
...
///   The reduction is still applied (clamped to the floor) so existing
...
pub fn apply_bad_debt_to_supply_index(cache: &mut Cache, bad_debt: Ray) {
    ...
    // Insolvency signal: drop of >90% in a single bad-debt event.
    // old / 10 vs new: if new < old/10 the index collapsed catastrophically.
    // The controller should subscribe to PoolInsolventEvent and disable the
    // asset/oracle — the pool itself has no status field to pause writes.
    ...
    // Floor at SUPPLY_INDEX_FLOOR_RAW to keep downstream math well-conditioned.
...
mod tests {
    ...
    impl TestSetup {
        fn new() -> Self {
            ...
            let contract = env.register(crate::LiquidityPool, (admin, params));
    ...
    // add_protocol_revenue_ray: skip accrual when supply_index is at/below the
    // safety floor — dividing by a near-zero index produces astronomical
    // scaled amounts that can overflow i128 (line 70).
    #[test]
    fn test_add_protocol_revenue_ray_skips_when_supply_index_below_floor() {
        let t = TestSetup::new();
        t.as_contract(|| {
            // supply_index set to (floor - 1) so the safety branch triggers.
            let mut cache = t.fresh_cache(PoolState {
                supplied_ray: 100 * RAY,
                borrowed_ray: 0,
                revenue_ray: 0,
    ...
    // apply_bad_debt_to_supply_index: zero total supply → short-circuit
    ...
    // apply_bad_debt_to_supply_index: bad_debt > total_supplied path (line 96)
    // — the cap clamps the debt and also triggers the insolvency branch +
    // floor clamp (lines 113-132).
    #[test]
    fn test_apply_bad_debt_caps_at_total_supply_and_triggers_insolvency() {
        ...
        t.as_contract(|| {
            ...
            // bad_debt > total_supplied → capped path + >90% reduction → event
            // emission + floor clamp (new index would be ~0, clamped to floor).
    ...
    // apply_bad_debt_to_supply_index: >90% reduction path emits
    // PoolInsolventEvent (lines 113-128) without requiring the debt to
    // exceed total supply — a 91% bad-debt event is enough.
    #[test]
    ...
    // apply_bad_debt_to_supply_index: mild (<90%) reduction — no event, no
    // floor clamp (exercises the "new_index >= floor" branch at line 134).
    #[test]
