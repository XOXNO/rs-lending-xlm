//! Protocol scales (RAY/WAD/BPS), time units, caps, oracle tolerances, TTL tiers.
...
/// Max spoke liquidation-curve target HF (WAD). Caps `target * debt` overflow risk.
pub const MAX_LIQUIDATION_TARGET_HF_WAD: i128 = 10 * WAD;

/// Default min LTV-weighted collateral (USD WAD) while an account carries debt.
...
// Shared/instance TTL: last-resort after keeper outage; prepaid at deploy + keeper top-up.
