...
**Scope:** the three core on-chain contracts — `contracts/governance`, `contracts/controller`, `contracts/pool`.

**Trust model:** the contract owner is a governance timelock backed by a multisig; listed tokens are standard Stellar Asset Contracts (SACs) with no transfer callback and full-value transfers, and fee-on-transfer or rebasing tokens are excluded by listing policy; oracle providers (Reflector, RedStone) and the DEX aggregator are governance-approved external dependencies that are not fully trusted; the off-chain indexer and TTL keeper are operated by the protocol team.
...
3. **Strategy / flash loan.** The user runs `multiply`, `swap_collateral`, `swap_debt`, `repay_debt_with_collateral`, `flash_loan`, or `migrate_from_blend`. Swaps route through the governance-approved DEX aggregator as opaque bytes; a reentrancy guard is held across every external callback.
...
3. **External oracle boundary** — Reflector and RedStone are called cross-contract. Return values are bounded, staleness-checked, and fail-closed. A single unanchored feed remains a trusted input for a market configured that way — see Spoof.1.
4. **External DEX / Blend boundary** — the aggregator and Blend pools are governance-approved addresses. The Controller passes the aggregator opaque swap bytes and measures its own balance delta; the aggregator enforces the slippage gate — see Elevation.3.
...
| **D**enial of Service | **DoS.1** — (Step 6) [High] A canceller can cancel any pending operation; if a role-revocation operation were cancellable a canceller could veto its own removal, and with no non-timelocked owner path to strip a role that would permanently freeze governance. `governance/timelock.rs`. <br /><br /> **DoS.2** — (Step 5) [Medium] Account risk totals price every leg; one stale or reverting feed blocks withdraw, borrow, `is_liquidatable`, and liquidation, so an underwater position touching a bad feed accrues bad debt. `risk/totals.rs:138-173`. <br /><br /> **DoS.3** — (Step 6) [Medium] `disable_token_oracle` removes the oracle entry; doing so with live positions bricks every position referencing the asset (repay only). `config/oracle.rs:109-117`. <br /><br /> **DoS.4** — (Step 6) [Medium] `pause` is owner-immediate with no timelock and `unpause` is owner-only, so a compromised owner key can halt the controller and withhold resume. `governance/timelock.rs:188-199`. <br /><br /> **DoS.5** — (Step 7) [Low] `Delegates(account_id)` is excluded from account TTL renewal and can archive independently, locking a delegate out. `storage/account.rs:215-227`. <br /><br /> **DoS.6** — (Step 6) [Low] A direct owner call can store an inverted or zero tolerance band (`lower > upper`) that makes every price read for the market revert. `config/oracle.rs:94-105`. <br /><br /> **DoS.7** — (Step 1–5) [Low] Batch entrypoints take unbounded `Vec`s; a position spanning many markets could exceed the ledger budget and become un-exitable in one transaction. `pool/src/lib.rs:326,522`. <br /><br /> **DoS.8** — (Step 7) [Low] Load-bearing persistent keys (account map, market params/state, hubs/spokes, oracle config) freeze pending a Soroban restore if their TTL lapses. `storage/ttl.rs:11-21`. |
| **E**levation of Privilege | **Elevation.1** — (Step 6) [Low] The constructor enforces only a non-zero delay, not `TIMELOCK_MIN_DELAY_LEDGERS = 34_560`, so a `min_delay = 1` deployment gives every standard admin operation a ~5-second delay. `governance/access.rs:151-152`. <br /><br /> **Elevation.2** — (Step 5) [Low] A registered delegate can liquidate the account it manages once health factor drops below one, moving the bonus from owner to delegate. `positions/liquidation/mod.rs:113-117`. <br /><br /> **Elevation.3** — (Step 3) [Informational] The Controller passes the approved aggregator opaque swap bytes; a compromised aggregator could underdeliver on a swap. `strategies/swap/mod.rs:22`. <br /><br /> **Elevation.4** — (Step 6) [Informational] The owner holds executor and canceller roles; the separation check runs only on delegated grants. `governance/access.rs:105-127`. |
...
- **Did the treatments identified in the "What are we going to do about it" section adequately address the issues identified?**
  - The High and both Medium code issues are addressed by local changes (an uncancellable marker on role revocations, a tolerance-band cap, and the aggregator's existing slippage gate). The two open Medium items (DoS.2, DoS.3) follow from the fail-closed oracle design; an oracle-free repay path provides a partial exit, and the fuller fixes require care so that liquidation robustness does not open a manipulation path.

- **Have additional issues been found after the threat model?**
  - None to date. The model will be updated as the contracts change.
