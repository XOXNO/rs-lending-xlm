...
8. **Controller/Pool ↔ Flash receiver** — cash `flash_loan` requires deployed
   ...
   measured collateral, and requires solvency (INV-STRAT-03, ADR-0020).
...
| | **Spoof.4** — A flash "receiver" that is a classic account rather than code. `require_wasm_receiver` rejects non-contract receivers on both `flash_loan` and `flash_position` (`contracts/controller/src/strategies/flash_loan.rs`, `flash_position.rs`). Allowance-repayment games apply only to I7; I8 `flash_position` cannot impersonate repayment because it never pulls principal back. Interactions: I7, I8. |
...
| **Spoof.4** | **R.1 ✅** `require_wasm_receiver` on cash `flash_loan` plus allowance-based repayment and pool balance assertions (INV-FLASH-01). **R.2 ✅** The same Wasm check on `flash_position`; repayment impersonation is not in scope because that path never collects principal (INV-STRAT-03). |
