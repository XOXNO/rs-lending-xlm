...
**Scope:** the three core on-chain contracts — `contracts/governance`, `contracts/controller`, `contracts/pool` — analyzed in depth, plus the boundaries to the first-party periphery contracts `contracts/aggregator` (swap router), `contracts/xoxno-oracle-adapter` (self-hosted N-of-M price feed), and `contracts/defindex-strategy` (reference vault adapter that opens/uses dedicated controller accounts). The internals of the periphery contracts (especially adapter signer model and defindex auth/harvest flows) warrant their own dedicated pass. Position-manager registry and accumulator are additional governance-approved surfaces.

**Trust model:** the contract owner is a governance timelock backed by a multisig; listed tokens are standard Stellar Asset Contracts (SACs) with no transfer callback and full-value transfers, and fee-on-transfer or rebasing tokens are excluded by listing policy; oracle providers (Reflector, RedStone, Xoxno adapter) are external/semi-trusted with fail-closed bounds, staleness, and sanity. The DEX aggregator router is first-party code (own admin) but treated as untrusted at the controller boundary (balance-delta verification per ADR 0005); external AMM venues are untrusted. The `xoxno-oracle-adapter` is first-party with N-of-M signer trust root and counts as a distinct provider. `defindex-strategy` is a first-party vault adapter (one dedicated controller account per vault). Position managers (governance-approved) and the revenue accumulator are additional surfaces. Off-chain indexer and TTL keeper are protocol-operated.
...
Additional governance surfaces: the **position-manager registry** (governance-approved entities that can act on delegated accounts) and the **revenue accumulator** (sink for protocol revenue). `defindex-strategy` is a first-party vault adapter that opens dedicated controller accounts.
...
flowchart TB
    ...
    subgraph periph["First-party periphery (own admin, semi-trusted at boundary)"]
        ...
        defi["DeFindex strategy<br/>(contracts/defindex-strategy)"]
    end
    ...
    defi -->|"supply/withdraw/harvest via controller accounts"| controller
    defi <-->|token flows| sac

    controller -.emits events.-> indexer
    pool -.emits events.-> indexer
    governance -.emits events.-> indexer
    defi -.emits HarvestEvent.-> indexer
...
3. **Oracle boundary** — Reflector, RedStone, and the first-party `xoxno-oracle-adapter` (distinct `Xoxno` provider with N-of-M signers). `PrimaryWithAnchor` requires non-spot primary (typically Reflector TWAP) + different provider/contract for anchor. `Single` allows spot with a tight per-asset sanity band (≤10% midpoint-relative). All resolution is fail-closed when a price is requested. Adapter internals (signer collusion, liveness) warrant separate analysis. A single unanchored feed is a trusted input for its market — see Spoof.1.
...
5. **Position manager / delegate boundary** — governance-approved position managers (plus owner delegates) can act on accounts. Once granted, they are powerful but still subject to the same risk gates and flash guard as owners.
6. **Accumulator boundary** — revenue forwarding sink; misconfiguration can lock or divert protocol fees.
7. **Off-chain boundary** — the indexer and keeper. Indexer fidelity depends on event coverage (Repudiate.*); the keeper's uptime backs the archival mitigation (DoS.8). Defindex-strategy introduces vault-level TTL and harvest flows.
...
| Threat | Issues |
| --- | --- |
| **S**poofing | **Spoof.1** — (Step 2, 5) [Low] For a `Single`-shaped market, a compromised or manipulated provider can return an in-bounds but wrong price. Defense is positivity, freshness, and a tight per-asset sanity band (≤10% midpoint-relative for Single; wider for anchored). Production `Single` does not force TWAP (spot allowed); anchored markets require non-spot primary + cross-provider. `governance/src/validate/oracle_config.rs`, `common/src/validation.rs:179+`. <br /><br /> **Spoof.2** — (Step 5) [Informational] The self-liquidation block compares only the owner address, so an owner can liquidate their own underwater account from a second address they control and take the bonus. `positions/liquidation/mod.rs:148-152`. |
...
| Threat | Issues |
| --- | --- |
| **S**poofing | **Spoof.1** — Single-market oracle provider is a trusted price source. <br /> **S1R1** — Mitigated for anchored markets (non-spot primary + cross-provider + tolerance). For `Single`, defense is a tight sanity band (≤10% midpoint-relative for production Single). The band can be further tightened per-asset. Adapter as price source adds its own N-of-M signer collusion / liveness risks (recommend dedicated sub-analysis). Position managers and defindex introduce additional authenticated actors that must be considered in spoofing/elevation paths. <br /><br /> **Spoof.2** — Self-liquidation via a second controlled address. <br /> **S2R1** — Accepted. Self-liquidation of an underwater position is economically neutral to the protocol. |
...
- **Any additional thoughts or insights on the threat modeling process that could help improve it next time?**
  - Re-run (or delta-review) the model whenever the oracle composition (including adapter as source), the delegation / position-manager model, the strategy and aggregator interface, the governance role set (including GUARDIAN), or first-party periphery (defindex-strategy, accumulator) changes. Material findings have historically concentrated in oracle, governance, strategies, and delegation surfaces.
  - Explicitly include defindex-strategy, position-manager registry, and accumulator in scope and data-flow on future passes. Adapter internals (N-of-M signers) and vault/harvest flows deserve dedicated sub-analysis.
