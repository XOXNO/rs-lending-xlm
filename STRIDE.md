...
3. **Oracle boundary** — Reflector and RedStone are external contracts; the first-party `xoxno-oracle-adapter` can serve the same read shapes with a protocol-operated N-of-M signer set as its trust root. Return values are bounded, staleness-checked, and fail-closed. A single unanchored feed remains a trusted input for a market configured that way — see Spoof.1.
