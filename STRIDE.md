...
| | **Tamper.7** — Malicious or accidental Wasm upgrade changes protocol behavior. Controller, pool, governance, and position-NFT upgrades are timelocked Sensitive operations; a controller upgrade forcibly re-enters the paused state (`upgrade` in `contracts/controller/src/governance.rs`). The Sensitive floor is currently 12 ledgers (about one minute), so the delay brake is nominal until 120_960 is restored. The price aggregator is immutable (no upgrade entrypoint; replacement only via timelocked `SetPriceAggregator` repoint). Router upgrades are timelocked as the Sensitive-tier `UpgradeSwapAggregator` only while the controller is the router's owner; with a standalone router owner (the `deploy-aggregator` default) they are owner-immediate. XOXNO oracle upgrades are owner-immediate — see Elevation.6/7. *Inherent: M×H = High. Residual: Medium (✅ + ⚙).* Interaction: I32. |
...
| **Tamper.7** | **R.1 ✅** Sensitive-tier timelock on controller/pool/governance upgrades, with payload identity binding the exact Wasm hash; the price aggregator is immutable by construction and replaceable only through the timelocked `SetPriceAggregator` repoint. **R.2 ✅** Controller upgrade forces the paused state, so post-upgrade activity requires an explicit, delayed unpause. **R.3 ⚙** Place the router's Ownable owner under the controller: that is what makes `UpgradeSwapAggregator` (Sensitive tier) the only upgrade path for it. Left on the `deploy-aggregator` default signer, the router upgrade stays owner-immediate. XOXNO oracle upgrades are owner-immediate with no governance route — accept and monitor them as hot trust (see Elevation.6/7). Verify the deployed owners, do not assume them. |
...
| **Elevation.6** | **R.1 ✅** Fee cap enforced at write time; sweep excludes reserved fee buckets; controller-side delta settlement bounds in-flight exposure to the stated input. **R.2 ⚙** Hold the router Ownable owner to the same custody standard as governance keys, or transfer it to the **controller** — the controller is the only owner value that makes the Sensitive-tier `UpgradeSwapAggregator` path reachable (`contracts/governance/src/op.rs:315` → `contracts/controller/src/markets.rs:123`). Transferring it to the governance contract would leave no upgrade path at all: no `AdminOperation` variant targets the router directly, so the only timelocked route runs governance → controller → router and needs the controller to pass the router's `#[only_owner]` gate. Monitor router upgrade events. |
