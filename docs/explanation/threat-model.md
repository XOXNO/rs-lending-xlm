...
boundaries to first-party periphery (`swap-aggregator`, `xoxno-oracle`,
`defindex-strategy`).
...
oracles (Reflector, RedStone, xoxno-oracle) fail-closed on bounds/staleness;
...
| DoS.9 | Info | Aggregator whitelist owner-gated, small set |
...
**DoS.9** — [Info] Aggregator whitelist is unbounded in code; owner-gated and
...
aggregator, position managers, or periphery (defindex, accumulator) change.
