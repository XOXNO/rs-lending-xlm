...
| Oracle config ownership | architecture, invariants, ADRs, skills, keeper notes | Token-rooted config is `AggregatorKey::AssetOracle(asset)` on **price-aggregator**; controller stores only `PriceAggregator` addr + cross-calls |
