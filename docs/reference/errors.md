...
| 219 `TwapInsufficientObservations` | Zero configured records, a record count below the smoothing minimum, excess history, spacing below provider resolution, or a history that does not span the window those records imply. | Supply a history spanning the configured window; a history holding fewer samples than records is accepted when the provider skipped a round, as long as it still spans that window. Reflector Twap computes an equal-weight mean. |
