...
| 102 | `HealthFactorTooLow` | After refreshing thresholds the health factor is below the minimum required to accept the update (`contracts/controller/src/risk/params.rs`). | Improve the health factor first. | controller `update_account_threshold` |
...
| 111 | `InvalidPositionMode` | The requested position mode is not Multiply, Long, or Short (`contracts/controller/src/strategies/multiply.rs:174`; `contracts/controller/src/strategies/flash_position.rs`). | Use Multiply, Long, or Short. | controller `multiply`, `flash_position` |
...
| 211 | `NoAccumulator` | No revenue accumulator address is configured on the controller (`contracts/controller/src/markets.rs`). | Wait for governance to set the accumulator. | controller `claim_revenue` |
...
| 301 | `SpokeDeprecated` | The spoke is flagged deprecated, or it is already deprecated when `remove_spoke` runs (`contracts/controller/src/context.rs`, `contracts/controller/src/config/spoke.rs:41`). A liquidation in `Credit(0)` mode is exempt: the seizure receiver may be created in a deprecated spoke (`contracts/controller/src/account.rs`, `SpokeAdmission::AllowDeprecated`). | Use an active spoke. | controller position entry points, `add_asset_to_spoke`, `remove_spoke` |
| 307 | `AssetNotInSpoke` | The hub asset is not listed on that spoke (`contracts/controller/src/context.rs`, `contracts/controller/src/config/asset.rs:66`, `:112`, `:158`). | List the asset on the spoke first. | controller position entry points, `edit_asset_in_spoke`, `set_spoke_asset_flags`, `remove_asset_from_spoke`, `get_spoke_asset` |
...
| 401 | `FlashloanNotEnabled` | The market's `is_flashloanable` parameter is false (`contracts/pool/src/ops/flash.rs:89`; `contracts/controller/src/strategies/flash_position.rs`, before the mint). | Choose a flash-loan-enabled market. | pool `flash_loan`; controller `flash_loan`, `flash_position` |
