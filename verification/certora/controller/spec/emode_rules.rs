...
fn emode_overrides_asset_params(e: Env, asset: Address, category_id: u32) {
    ...
    let mut cache = crate::cache::ControllerCache::new(&e, crate::oracle::policy::OraclePolicy::RiskIncreasing);
    let emode_asset_cfg =
...
/// `category_id` from each member market's reverse e-mode category vector.
/// The rule asserts:
///   1. category flagged deprecated;
///   2. category asset map empty after the call;
///   3. for the sampled pre-existing member, the reverse index
///      no longer contains `category_id`;
///   4. when the sampled member belonged only to this category, its reverse
///      index becomes empty.
#[rule]
fn emode_remove_category(e: Env, category_id: u32) {
    ...
    // Capture the pre-state reverse-index length so the post-condition can
    // target the orphaned-member branch.
    ...
    // (4) When the sampled member's reverse index had only this category,
    // the current market config must carry an empty reverse index after
    // removal.
