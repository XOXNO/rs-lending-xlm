//! Regression for the one-sided TTL expiration bug. The fix in
//! `controller/src/storage/account.rs::write_side_map` renews *both*
//! side keys on every write, not just the side being mutated. Without
//! the fix, a borrower who mutates only the supply side (e.g. adding
//! collateral repeatedly) would let the borrow-side key age out and
//! get archived independently of the still-live supply side.
