//! Max-utilization ceiling regression (Blend V2 H-03 / M-14;
//! `audit-research/STELLAR_AUDIT_FINDINGS.md` §4.3).
//!
//! Default preset cap is 95 % (`RAY * 95 / 100`). Borrow / withdraw /
//! strategy borrow paths revert with `UtilizationAboveMax` when the
//! post-state utilization would exceed the cap. Liquidation seize
//! intentionally bypasses (the protocol must always be able to liquidate).
