//! Per-`ContractCostType` CPU-instruction breakdown for the controller's hot
//! mutating ops. Built to ANSWER ONE QUESTION before optimizing: is the
//! per-asset mutation cost dominated by the interest-accrual `Ray`/I256 math,
//! by storage I/O, or by ScVal marshaling?
//!
//! ## What this harness measures accurately, and what it does NOT
//! Native tests run the contract as native Rust, so `WasmInsnExec`,
//! `VmInstantiation`, `VmCachedInstantiation`, `InvokeVmFunction` and the
//! `ParseWasm*`/`InstantiateWasm*` families are UNDER-metered here (the SDK
//! warns about this directly). Those are the VM/cross-contract-boundary costs;
//! they are paid on-chain per cross-contract call and were measured separately
//! on the router suite + testnet simulate. The HOST cost types this harness
//! cares about — `Int256AddSub/Mul/Div/Pow/Shift` (the interest math),
//! `Mem*`, `Val*` (ScVal ser/deser at the host boundary), `VisitObject`,
//! `DispatchHostFunction` — are metered accurately. So the RELATIVE composition
//! among host cost types is sound; absolute totals are a floor, not the
//! on-chain number.
//!
//! Run with: `cargo test -p test-harness --test budget_breakdown -- --nocapture`
