//! Boundary regressions for liquidation eligibility, bonus interpolation,
//! and the bad-debt threshold. Existing `liquidation_math_tests.rs` covers
//! the math for moderate / deep underwater positions; this file focuses
//! on the *boundaries* — HF≈1 exact / 1 ulp under, bonus monotonicity
//! across worsening HF, and the $5 bad-debt threshold edge.
...
// Even at the deepest liquidatable level, the bonus is clamped at
// MAX_LIQUIDATION_BONUS (15 %). Pins Slender H-1 absence: bonus never
// reaches the zero-debt-seizure regime.
#[test]
fn test_liquidation_bonus_clamped_at_max() {
    ...
    // MAX_LIQUIDATION_BONUS = 1500 bps = 15 %. Allow small arithmetic
    // slop on the upper bound; the gate is "no runaway profit".
    assert!(
        realized_bonus <= 0.16,
        "realized bonus must be clamped at max (~15 %), got {:.4}",
        realized_bonus
    );
}
