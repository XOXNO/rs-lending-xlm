//! E-mode liquidation behaviour at boundaries and across category-asset
//! combinations. `emode_tests.rs` covers the happy path (enhanced
//! LTV/threshold, e-mode bonus); this file pins:
//!
//! 1. HF computation uses the e-mode threshold (not the asset's standard
//!    threshold) — a position that would be liquidatable under the
//!    asset's 80 % threshold remains healthy under the e-mode 98 %
//!    threshold.
//! 2. E-mode bonus is consistent across HF in the mild-underwater band
//!    and stays bounded above by the e-mode max (not the protocol max).
//! 3. Two-asset same-category liquidation: collateral seized correctly
//!    when both collateral and debt sides are e-mode assets.
